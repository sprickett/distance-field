...
bool load_bitmap(std::ifstream& f, std::vector<uint8>& data, int& width, int& height,int& stride, int& bits_per_pixel)
{
	...
	// cheekily forget about palette and dib
...
int main(int argc, char* argv[])
{
	...
	if (argc > 1 && load_bitmap(argv[1], data, width, height, stride,bits_per_pixel)
			&& bits_per_pixel == 8 
			&& gen(data.data(), data.data(), 128, width, height, stride,stride)
			&& save_bitmap("df.bmp", data, width, height, stride, bits_per_pixel))
	{
		printf("saved\n");
	}
